-- Notify authorized internal recipients when a Service Request is created.
...
drop trigger if exists service_request_created_notification
on public.service_requests;

create trigger service_request_created_notification
after insert on public.service_requests
for each row
execute function public.notify_staff_of_new_service_request();
...
comment on function public.notify_staff_of_new_service_request() is
  'Creates idempotent, tenant-scoped Communications notifications for authorized internal Service Desk recipients after a Service Request insert.';

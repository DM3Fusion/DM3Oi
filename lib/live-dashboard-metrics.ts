...
const taskTerminal = new Set(["COMPLETED", "NOT_APPLICABLE"]);
...
export function getOperationalDashboardMetrics(cases:LiveCase[],serviceRequests:LiveServiceRequest[],customers:number,unreadCommunications:number,timezone:string,now=new Date()){
  ...
  const isOpenTask=(status:string)=>!taskTerminal.has(status);
  const dueToday=tasks.filter(task=>task.due_at&&isOpenTask(task.status)&&new Date(task.due_at)>=dayStart&&new Date(task.due_at)<nextDay).length;
  const overdueTasks=tasks.filter(task=>task.due_at&&isOpenTask(task.status)&&new Date(task.due_at)<dayStart).length;
  const openTasks=tasks.filter(task=>isOpenTask(task.status)).length;
  ...
  return {
    kpis:[
      {label:"Active Cases",value:activeCases,href:"/cases",detail:"Current authorized caseload",tone:"blue"},
      {label:"Open Tasks",value:openTasks,href:"/tasks",detail:"Not completed or excluded",tone:"cyan"},
      {label:"Due Today",value:dueToday,href:"/tasks",detail:"Organization-local date",tone:dueToday?"amber":"slate"},
      {label:"Open Service Requests",value:openRequests,href:"/service-desk",detail:"Active Service Desk workload",tone:"violet"},
    ...
    caseProgress:[
      {label:"New",value:cases.filter(item=>item.status==="NEW").length},
      {label:"Assigned",value:cases.filter(item=>["UNASSIGNED","ASSIGNED"].includes(item.status)).length},
      {label:"In Progress",value:cases.filter(item=>["IN_PROGRESS","REVIEW"].includes(item.status)).length},
      {label:"Waiting",value:cases.filter(item=>item.status==="WAITING").length},
      {label:"Completed",value:cases.filter(item=>["COMPLETED","CLOSED"].includes(item.status)).length},
    ],
    ...
    attention:[
      {label:"Overdue tasks",value:overdueTasks,href:"/tasks",tone:"red"},
      {label:"Tasks due today",value:dueToday,href:"/tasks",tone:"amber"},

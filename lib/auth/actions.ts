...
export async function verifyLoginOtpAction(formData:FormData){const email=read(formData,"email").toLowerCase();const token=read(formData,"token").replace(/\s+/g,"");const next=safeInternalPath(read(formData,"next"));if(!isSupabaseConfigured())redirect(loginUrl({error:"Supabase environment variables are not configured.",email,next}));if(!email||!token)redirect(loginUrl({sent:"1",error:"Enter the email code.",email,next}));const supabase=await createClient();const {error}=await supabase.auth.verifyOtp({email,token,type:"email"});if(error)redirect(loginUrl({sent:"1",error:"The email code is invalid or has expired.",email,next}));redirect(next);}

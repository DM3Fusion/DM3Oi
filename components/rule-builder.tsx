...
type BuilderAction={id?:string;action_type:ActionType;target_question_id:string;task_title:string;task_description:string;task_priority:Database["public"]["Enums"]["priority_level"];task_required:boolean;task_blocking:boolean;task_due_in_days:number};
...
const blankAction=(questions:RuleQuestion[],source:string):BuilderAction=>({action_type:"SHOW_QUESTION",target_question_id:questions.find(q=>q.active&&q.id!==source)?.id??"",task_title:"",task_description:"",task_priority:"NORMAL",task_required:true,task_blocking:false,task_due_in_days:7});
...
export function RuleBuilder({questions,rule}:{questions:RuleQuestion[];rule?:RuleDefinition}) {
  ...
  return <>
    <button type="button" className={rule?"secondary-button":"primary-button"} onClick={()=>dialog.current?.showModal()}>{rule?"Edit":<><ApplicationIcon name="add"/>Add Rule</>}</button>
    <dialog ref={dialog} className="rule-builder-dialog" aria-labelledby={titleId} onCancel={event=>{event.preventDefault();dialog.current?.close();}}>
      <form action={saveRuleAction} className="rule-builder-form">
        <header><div><p className="eyebrow">Rule Builder</p><h2 id={titleId}>{rule?"Edit Rule":"Add Rule"}</h2></div><button type="button" className="rule-dialog-close" aria-label="Close Rule Builder" onClick={()=>dialog.current?.close()}><ApplicationIcon name="close"/></button></header>

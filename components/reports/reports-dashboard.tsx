...
export function ReportsDashboard({ report }: { report: OperationalReport }) {
  ...
  return <div className="reports-dashboard">
    ...
    {availableKpis.length ? <section className="report-kpis" aria-label="Historical performance summary">{availableKpis.map((kpi) => {
      ...
      {capabilities.cases ? <div className="report-grid">
      ...
      <div className="report-grid report-three">
        ...
        <section className="panel report-panel"><div className="section-head"><div><h2>Operational Bottlenecks</h2><p>Current blocked or overdue Task patterns</p></div>{capabilities.tasks ? <Link href="/tasks?status=blocked">View blocked <ApplicationIcon name="forward" /></Link> : null}</div>{capabilities.tasks ? <BarList rows={report.bottlenecks.map((item) => ({ key: item.key, label: item.label, value: item.count }))} empty="No current blocked or overdue Task patterns." /> : <div className="no-results">Bottleneck detail requires Task access.</div>}</section>

...
export function RuleFilters({q,status}:{q:string;status:RuleStatusFilter}) {
  ...
  return <form className="rule-filters" aria-label="Filter Rules" onSubmit={event=>event.preventDefault()}><label><span className="sr-only">Search Rules</span><span className="rule-search-control"><ApplicationIcon name="search" className="search-field-icon"/><input type="search" value={search} onChange={event=>change(event.currentTarget.value)} placeholder="Search Rules..." maxLength={200}/>{search?<button type="button" aria-label="Clear Rule search" onClick={()=>{setSearch("");update("");}}><ApplicationIcon name="close"/></button>:null}</span></label><label className="rule-status"><span>Status</span><select value={status} onChange={event=>update(search,event.currentTarget.value as RuleStatusFilter)}><option value="all">All</option><option value="active">Active</option><option value="inactive">Inactive</option></select></label></form>;
}

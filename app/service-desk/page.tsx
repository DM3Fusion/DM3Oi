  ...
  return (
    <>
      ...
      <div className="metric-grid service-desk-metrics">
        {metrics.map((metric) => (
          <Link key={metric.label} href={metric.href} className={`metric tone-${metric.tone}`}>
            ...
            <table>
              ...
              <tbody>
                {data.serviceRequests.slice(0, 8).map((request) => (
                  <NavigableRow key={request.id} href={`/service-desk/${request.id}`} label={`Open service request ${request.request_number}`}>
                    <td>
                    ...
                    <td>{formatServiceRequestUpdatedAt(request.updated_at, data.timezone)}</td>
                  </NavigableRow>
                ))}
              </tbody>
            </table>

...
export default async function Page({
  ...
  return <><PageHeader eyebrow="Administration" title="Organization Defaults" description="Set safe organization-wide operational defaults."/>{query?.error&&<div className="form-alert page-notice">{query.error}</div>}{query?.message&&<div className="success-alert page-notice">{query.message}</div>}<section className="panel form-panel"><OrganizationDefaultsForm action={saveOrganizationDefaults} initial={{postal:organization?.business_postal_code??"",mode:timezoneSource==="MANUAL"?"MANUAL":"ZIP",timezone:settings?.timezone??"UTC",priority:settings?.default_priority??"NORMAL"}} saved={query?.message==="Changes Saved"}/></section></>;
}

...
export default async function RootLayout({children}:{children:React.ReactNode}){const access=await getAccessContext();const unreadNotificationCount=access?.activeOrganization&&access.internalAccess?await getUnreadNotificationCount({organizationId:access.activeOrganization.id,userId:access.user.id}):0;return <html lang="en"><body><NavigationFeedback/><AppShell access={access} applicationVersion={getApplicationVersion()} unreadNotificationCount={unreadNotificationCount}>{children}</AppShell></body></html>}

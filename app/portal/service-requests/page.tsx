import Link from "next/link";
import { requireCustomerPortalContext } from "@/lib/auth/customer-portal";
import { customerPortalDataOrThrow } from "@/lib/data/customer-portal-query";
import { createClient } from "@/lib/supabase/server";

export default async function PortalRequestsPage() {
  const context = await requireCustomerPortalContext();
  const supabase = await createClient();
  const result = await supabase
    .from("organization_service_requests")
    .select("id,request_number,subject,status,updated_at")
    .eq("organization_id", context.organization.id)
    .eq("customer_id", context.customer.id)
    .order("updated_at", { ascending: false });
  const requests = customerPortalDataOrThrow(
    result,
    "service-request-list",
  ) ?? [];

  return <section className="portal-panel"><div className="portal-title"><div className="portal-service-requests-heading"><p className="eyebrow">{context.customer.name}</p><h1>Service Requests</h1></div><Link className="primary-button" href="/portal/service-requests/new">New Service Request</Link></div>{requests.length ? <ul className="portal-request-list portal-service-requests-list">{requests.map((request) => <li key={request.id}><Link href={`/portal/service-requests/${request.id}`}><span>{request.request_number}</span><strong>{request.subject}</strong><span className="portal-request-meta">{request.status.replaceAll("_", " ")}</span></Link></li>)}</ul> : <p>No service requests yet.</p>}</section>;
}

      ...
      <div className="detail-grid">
        <div className="detail-main">
          ...
          <section className="panel detail-section">
            <div className="section-head">
              <h2>Tasks</h2>
              <div className="case-task-heading-actions"><span className="count-pill">
                {item.progress.remainingWork.filter((work) => work.kind === "TASK").length} remaining
              </span>{canManage ? <CreateTaskModal caseId={item.id} staff={data.staff.map((staff) => ({ id: staff.profile.id, name: displayName(staff.profile) }))} /> : null}</div>
            </div>
            {item.tasks.length ? (
